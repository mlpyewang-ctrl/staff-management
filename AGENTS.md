...
Do not commit local secrets from `.env`. If you change Prisma models, update the database workflow and mention whether `db:push` or `db:seed` is required. Deployment scripts are already in `scripts/`; reuse them instead of creating new ad hoc flows.

- **Offline native deployment** (`scripts/build-offline-native.ps1`): the only supported deployment method. Run the PowerShell script on a Windows machine with internet to install production dependencies, build the app, and create a `tar.gz` bundle. Copy the bundle to the offline CentOS 7.9 (or compatible) server, install Node.js from [unofficial-builds.nodejs.org](https://unofficial-builds.nodejs.org/) (`linux-x64-glibc-217`), configure `.env` (pointing `DATABASE_URL` to the external PostgreSQL instance), and run `npm start`.
- The project uses `bcryptjs` (pure JS) instead of `bcrypt` (native C++), so `node_modules` copied from Windows works directly on Linux.
- Prisma is configured with `binaryTargets = ["native", "rhel-openssl-1.0.x"]` so the query engine for CentOS 7 is already included in the bundle.
- Docker deployment has been removed; do not reintroduce Dockerfiles or compose files.

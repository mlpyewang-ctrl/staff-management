...
- **Offline Docker deployment** (`scripts/build-offline-bundle.ps1` → `scripts/deploy-offline.sh`): on Windows with internet, run the PowerShell script to build and export the app Docker image. Copy the resulting bundle to the offline server, configure `.env.prod` (pointing `DATABASE_URL` to the external PostgreSQL instance), and run `scripts/deploy-offline.sh` to deploy the app container with pure `docker run`.
- **Offline native deployment** (`scripts/build-offline-native.ps1`): for CentOS 7.9 or other Linux servers without Docker. Run the PowerShell script on Windows to install dependencies, build the app, and create a zip bundle. Copy the bundle to the offline server, install Node.js from [unofficial-builds.nodejs.org](https://unofficial-builds.nodejs.org/) (`linux-x64-glibc-217`), configure `.env`, and run `npm start`. The project uses `bcryptjs` (pure JS) instead of `bcrypt` (native C++), so `node_modules` copied from Windows works directly on Linux. Prisma is configured with `binaryTargets = ["native", "rhel-openssl-1.0.x"]` so the query engine for CentOS 7 is already included in the bundle.
